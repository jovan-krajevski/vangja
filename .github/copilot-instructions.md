...
- `load_kaggle_temperature(city, start_date, end_date, freq)` — Hourly temperature data for 36 cities (2012-2017) from Kaggle. Returns Celsius. Supports temporal aggregation via `freq` (e.g. `"D"`, `"W"`, `"h"`). Requires `kagglehub` (install with `pip install vangja[datasets]`)
- `load_smart_home_readings(column, start_date, end_date, freq)` — Smart home appliance energy readings at 1-minute resolution (~2016). Column can be an appliance (`"Fridge [kW]"`) or a total (`"use [kW]"`). Supports temporal aggregation via `freq`. Requires `kagglehub` (install with `pip install vangja[datasets]`)
