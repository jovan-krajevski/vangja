...
RESULTS_FOLDER = Path(__file__).parent / "results" / "timeseers"
...
lt_shrinkage_strength = [1, 100, 10000]
fs_shrinkage_strength = [1, 100, 10000]
slope_sd = [5.0]
intercept_sd = [5.0]
beta_sd = [1.0, 5.0, 10.0]
scalers = ["maxabs"]
delta_side = ["left", "right"]
...
for ws, uc, lt_ss, fs_ss, ssd, icd, bsd, scl, ds in params:
    experiments.add(
        Experiment(
            window_size=ws,
            uniform_constant=uc,
...
for start_date in start_dates:
    ...
    for i, experiment in enumerate(sorted_experiments):
        ...
        print(
            f"Training model with window_size={experiment.window_size}, "
            ...
            f"delta_side={experiment.delta_side}"
        )
